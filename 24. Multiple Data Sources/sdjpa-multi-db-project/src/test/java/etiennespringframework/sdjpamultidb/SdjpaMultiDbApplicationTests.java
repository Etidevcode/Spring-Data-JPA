package etiennespringframework.sdjpamultidb;

import etiennespringframework.sdjpamultidb.domain.creditcard.CreditCard;
import etiennespringframework.sdjpamultidb.services.CreditCardService;
...
class SdjpaMultiDbApplicationTests {
    ...
    void testSaveAndGetCreditCard() {
        ...
        CreditCard savedCC = cardService.saveCreditCard(cc);

        assertThat(savedCC).isNotNull();
        assertThat(savedCC.getId()).isNotNull();
        assertThat(savedCC.getCreditCardNumber()).isNotNull();

        CreditCard fetchedCreditCard = cardService.getCreditCardById(savedCC.getId());
